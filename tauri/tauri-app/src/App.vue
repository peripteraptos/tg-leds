...
// --- video selection (Tauri file dialog) ---
async function selectVideo() {
  try {
    ...
    if (typeof selected === "string") {
      ...
      videoUrl.value = convertFileSrc(selected);
      console.log("Selected video:", selected, videoUrl.value);
...
async function runRustExtraction() {
  ...
  try {
    ...
    // call Tauri command
    await invoke("extract_sequences", { config: req });
